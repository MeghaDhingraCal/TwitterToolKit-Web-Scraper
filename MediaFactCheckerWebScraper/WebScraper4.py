...
cursor.execute("CREATE TABLE scraped30 (source_name TEXT, domain_name TEXT, fact_check_score INTEGER, bias_score INTEGER)")
...
for ke in diction.keys():
    ...
    cursor.execute("INSERT INTO scraped30 (source_name, domain_name, fact_check_score, bias_score) VALUES (?,?,?,?)", [sour, dom, fc, bi])

connection.commit()
for row in connection.execute('SELECT * FROM scraped30'):
    print(row)
